...
SEPA_STATS_PK = ['CISLO_UCTU','MENA','CISLO_VYPISU','DATUM_START','DATUM_END']
SEPA_DATA_PK = []
...
class CEB_txt_parser:
    ...
    def parse_sepa(file_path, output_path):
        ...
        stats_output_path = os.path.join(output_path,KEY_VYPIS+ 'stats', file_name + '-stats.csv')
        if not os.path.exists(os.path.dirname(stats_output_path)):
            os.makedirs(os.path.dirname(stats_output_path))

        data_output_path = os.path.join(output_path,KEY_VYPIS+'data', file_name + '-data.csv')
        ...
        with open(file_path, encoding='windows-1250') as input_file:
            with open(stats_output_path, 'w+', newline='', encoding='utf-8') as stats_out:
                ...
                if (line[0] != KEY_HEADER_ROW):
                    raise ValueError("First line of file [{}] does not contain header".format(file_name)) 
                ...
                date_start = line[5]
                date_end = line[7]
                stats_pk = hashlib.md5('|'.join([c_ucet,mena,nr_vypis,date_start,date_end]).encode(encoding='utf_8')).hexdigest()
                line += [stats_pk]
                # write data
            ...
            with open(data_output_path, 'w+', newline='', encoding='utf-8') as data_output_file:
                ...
                for row in input_file:
                    ...
                    if(values[0] == KEY_DATA_ROW):
                        ...
                        raise ValueError('Unsupported type: {}'.format(values[0]))
